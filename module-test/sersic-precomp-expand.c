...
# 3 "../src/cps/sersic.c" 2
# 1 "../src/cps/../component.h" 1
...
# 4 "../src/cps/sersic.c" 2
# 1 "../src/cps/../recipe.h" 1
...
# 9 "../src/cps/sersic.c" 2
# 1 "/usr/include/utils.h" 1 3 4
...
cps_sersic_2d(double * par, double * par_lim, int * is_const, const char * name)
{
  ...
  component * cp_t = make_base_component(16);


  for(I_par = 0; I_par < 16; ++ I_par)
    *(cp_t -> par + I_par) = *(par + I_par),
    *(cp_t -> is_const + I_par) = *(is_const + I_par);


  for(I_par = 0; I_par < 2 * 16; ++ I_par)
...
_cps_sersic_2d_sigma(double x, double y, double const * par)
{
  ...
  double cos_rt = x / r_t, sin_rt = y / r_t,
         cos_pt = cos((par[ 3])), sin_pt = sin((par[ 3]));
  double x_t = r_t * (cos_rt * cos_pt + sin_rt * sin_pt),
         y_t = r_t * (sin_rt * cos_pt - cos_rt * sin_pt);


  double r = pow( pow(fabs(x_t - (par[ 1])), (par[ 7]))
                + pow(fabs(y_t - (par[ 2])) / (par[ 6]), (par[ 7])), 1. / (par[ 7]));


  double n = (par[ 5]), n_sq = (((par[ 5])) * ((par[ 5])));
  double dn = 3. * n - (1. / 3.) + (8. / 1215.) / n
            + (184. / 229635.) / n_sq + (1048. / 31000725.) / (n * n_sq)
            - (17557576. / 1242974068875.) / ((n_sq) * (n_sq));


  return (par[ 0]) * exp(-dn * (pow(r, 1. / n) - 1.));
}
    ...
    double const * par, double * sigma)
{
  ...
  double cos_pt = cos((par[ 3])), sin_pt = sin((par[ 3]));

  FOREACH(I_pt, N_pt)
    r_t[I_pt] = sqrt(((x[I_pt]) * (x[I_pt])) + ((y[I_pt]) * (y[I_pt])));
  ...
  FOREACH(I_pt, N_pt)
    r[I_pt] = pow( pow(fabs(x_t[I_pt] - (par[ 1])), (par[ 7]))
                 + pow(fabs(y_t[I_pt] - (par[ 2])) / (par[ 6]), (par[ 7])), 1. / (par[ 7]));
  ...
  FOREACH(I_pt, N_pt) sigma[I_pt] = (par[ 0]) * exp(-dn * (pow(r, 1. / n) - 1.));
    ...
    recipe * rcp_t, double const * par)
{
  ...
  double r = pow( pow(abs(x_t - (par[ 1])), (par[ 7]))
                + pow(abs(y_t - (par[ 2])) / (par[ 6]), (par[ 7])), 1. / (par[ 7]));
  ...
  double sigma = (par[ 0]) * exp(-dn * (pow(r, 1. / n) - 1.));
  ...
  double c_age = (par[ 8]) + (par[12]) * r, s_age_sq = (((par[ 9]) + (par[13]) * r) * ((par[ 9]) + (par[13]) * r)),
         c_Z = (par[10]) + (par[14]) * r, s_Z_sq = (((par[11]) + (par[15]) * r) * ((par[11]) + (par[15]) * r));
  ...
  for(I_age = 0; I_age < N_age; ++ I_age)
    for(I_Z = 0; I_Z < N_Z; ++ I_Z)
      *(rc + I_age * N_Z + I_Z) =
            exp(-((age_ax[I_age]) * (age_ax[I_age])) / (2. * s_age_sq))
          * exp(-((Z_ax[I_Z]) * (Z_ax[I_Z])) / (2. * s_Z_sq));
